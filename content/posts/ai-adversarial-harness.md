...
让 AI 完成复杂任务的核心方法是对抗分离：一个 AI 负责生成，另一个专门评估和挑错。Anthropic 的实验显示，单 Agent 花 $9 做 20 分钟产出的项目核心功能不可用，而对抗架构花 $200 做 6 小时产出的项目功能完整。关键不是模型更强，是让做事的和评判的分开，用张力驱动质量。

22 倍的成本差距，但换来的不是"好一点"，是"能用"和"不能用"的区别。

**你能拿走的**：<mark>用两个对话完成一个任务</mark>。第一个对话让 AI 做事，做完后把成果复制到第二个全新的对话，告诉它"你是严格的审查者，请找出以下内容的所有问题"。你会惊讶于第二个 AI 能找到多少第一个视而不见的缺陷。
...
这一步能省掉后面大量返工。你提前和 AI 对齐了"做完"的定义，后续每一步都有明确的验收标准，不再是做完之后含糊地问一句"你觉得怎么样"。
...
今天就能做的一件事：下次让 AI 帮你做复杂任务时，做完后别急着用。开个新对话，让另一个 AI 当审查者，告诉它你的验收标准。只这一步，输出质量就会有肉眼可见的提升。
