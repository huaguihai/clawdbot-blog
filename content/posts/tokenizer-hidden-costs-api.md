...
Playcode 对同一批 TypeScript 样本调用各家的官方计数接口后发现：字节数没有变化，计费结果却明显不同——**Anthropic 的新 Tokenizer，把同一段 TypeScript 切出了 GPT 的 1.73 倍 Token**。
...
这种成本变化很容易被误判成调用次数统计错误：调用链路没有变化，模型版本一换，毛利率却慢慢往下拐。只有把同一批请求交给各家的官方计数接口，才能把 Tokenizer 导致的增量单独拆出来。
...
另一种可测试的路由方式是：用 Claude 做高层决策，GPT 处理代码密集型任务。这种组合可能出现风格割裂，是否省钱也取决于上下文结构，因此要用自己的样本和质量指标做 A/B 对比，不能直接照搬结论。
...
例如，一个 prompt 文件从 2k 增长到 6k Token，如果 CI 只检查文件大小而不检查各模型的计数结果，这种成本回归很容易漏掉。**Token 应该像内存和 CPU 一样被持续监控，而不是月底看到云账单后再追查。**
